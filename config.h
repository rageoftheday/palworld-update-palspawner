...
public:
	...
	static bool IsAlive(SDK::APalCharacter* pChar);
	static bool IsABaseWorker(SDK::APalCharacter* pChar, bool bLocalControlled);
	static void Init();
	static void Update(const char* filterText);
	static const std::vector<std::string>& GetFilteredItems();
	static bool IsInGame(SDK::UObject* world);
