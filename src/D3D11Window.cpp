...
namespace DX11_Base {
	...
	bool D3D11Window::Hook()
	{
		if (InitHook()) {
			...
			Sleep(1000);
#if DEBUG
			g_Console->printdbg("D3D11Window::Hook Initialized %llx || 0x%llx\n", Console::Colors::pink, WindowHwnd, oIDXGISwapChainPresent);
	...
	bool D3D11Window::InitWindow()
	{
		...
		WindowHwnd = CreateWindow(WindowClass.lpszClassName, L"OverLay Window", WS_OVERLAPPEDWINDOW, 0, 0, 100, 100, NULL, NULL, WindowClass.hInstance, NULL);
		if (WindowHwnd == NULL) {
			return FALSE;
		}
#if DEBUG
		g_Console->printdbg("D3D11Window::OverLay Window Created ||0x%llx\n", Console::Colors::pink, WindowHwnd);
#endif
		return TRUE;
	}
	...
	bool D3D11Window::Init(IDXGISwapChain* swapChain)
	{
		if (SUCCEEDED(swapChain->GetDevice(__uuidof(ID3D11Device), (void**)&m_Device))) {
			...
			io.Fonts->AddFontFromFileTTF("c:/windows/Fonts/FRAMD.TTF", 18.0f, nullptr,io.Fonts->GetGlyphRangesChineseFull());
...
#if DEBUG
			g_Console->printdbg("D3D11Window::Swapchain Initialized || 0x%llx\n", Console::Colors::pink, swapChain);
#endif
			return 1;
		}
